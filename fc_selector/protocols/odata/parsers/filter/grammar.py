"""
OData Filter Expression Parser using SLY (Sly Lex-Yacc).

Implementation of a subset of the OData formal grammar for $filter expressions.
Based on: https://docs.oasis-open.org/odata/odata/v4.01/csprd06/abnf/odata-abnf-construction-rules.txt
Implemented with SLY: https://sly.readthedocs.io/en/latest/
...
class ODataParser(Parser):
    ...
    # Function calls
    ####################################################################################
    def _function_call(self, func: ast.Identifier, args: list[ast.Node]):
        ":meta private:"
    ...
    def _reverse_attributes(self, attr: ast.Attribute) -> ast.Attribute:
        ...
        owner: ast.Identifier | ast.Attribute = ast.Identifier(exploded.pop(0))
