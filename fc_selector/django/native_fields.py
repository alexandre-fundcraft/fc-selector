...
def parse_expand_fields(expand_string: str) -> dict[str, str | None]:
    """
    ...
    for char in expand_string + ",":
        if char == "(" and paren_depth == 0:
            ...
            if current_field.strip():
                field_name, nested_select = _parse_single_expand_field(
                    current_field.strip()
                )
...
def _parse_single_expand_field(field: str) -> tuple[str, str | None]:
    """
...
class NativeFieldExpansionMixin:
    ...
    def _apply_field_expansion(self):
        ...
        for field_name, options in expand_fields.items():
            ...
            serializer_class, serializer_options = self._parse_serializer_config(
                serializer_config
            )
            ...
            self._expanded_fields_data[field_name] = {
                "odata_params": nested_odata_params
            }

            # Instantiate the related serializer and add to fields
            try:
                self.fields[field_name] = serializer_class(
                    context=nested_context, **serializer_options
                )
                logger.debug(
                    f"Expanded field '{field_name}' with {serializer_class.__name__}"
                )
            except Exception as e:
                logger.error(f"Error expanding field '{field_name}': {e}")

    def _parse_serializer_config(self, config: Any) -> tuple[type, dict[str, Any]]:
        """
