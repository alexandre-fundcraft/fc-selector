...
def iter_dataclass_fields(node: nodes._Node) -> Iterator[tuple[str, Any]]:
    """
...
class NodeVisitor:
    ...
    def generic_visit(self, node: nodes._Node):
        """
        ...
        for field, value in iter_dataclass_fields(node):
            if isinstance(value, list):
                for item in value:
                    if isinstance(item, nodes._Node):
                        self.visit(item)
            elif isinstance(value, nodes._Node):
                self.visit(value)
...
class NodeTransformer(NodeVisitor):
    ...
    The visitor methods should return instances of nodes._Node that replace
