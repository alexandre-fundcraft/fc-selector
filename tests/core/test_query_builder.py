...
class TestODataQueryBuilderInit:
    """Tests for ODataQueryBuilder initialization."""
...
class TestODataQueryBuilderExpand:
    ...
    def test_expand_multiple(self):
        query = ODataQueryBuilder().expand("author", "categories")
...
class TestODataQueryBuilderBuild:
    ...
    def test_build_query_string_with_filter(self):
        query = ODataQueryBuilder().filter("Price gt 100")
        assert query.build_query_string() == "$filter=Price gt 100"

    def test_build_query_string_from_existing(self):
        query = ODataQueryBuilder("$select=id,name").filter("Price gt 100")
        result = query.build_query_string()
        assert "$filter=Price gt 100" in result
        assert "$select=id,name" in result

    def test_to_dict(self):
        query = ODataQueryBuilder().filter("status eq 'active'").select("id", "name").top(10)
...
class TestODataQueryBuilderChaining:
    ...
    def test_from_query_string_and_modify(self):
        ...
        query = ODataQueryBuilder.from_query_string("$select=id,name&$top=10")
...
class TestODataQueryBuilderStringRepresentation:
    ...
    def test_repr(self):
        query = ODataQueryBuilder().filter("status eq 'active'")
        assert "ODataQueryBuilder" in repr(query)
