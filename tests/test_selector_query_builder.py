"""
Tests for ODataSelector with QueryBuilder integration.
...
class TestSelectorGetOne:
    ...
    def test_get_one_by_id_filter(self):
        ...
        query = QueryBuilder().and_filter(f"id eq {obj.id}")
    ...
    def test_get_one_not_found(self):
        ...
        selector = ODataTestSelector()
        query = QueryBuilder().and_filter("id eq 99999")
        dto = selector.get_one(query)
...
class TestSelectorGetMany:
    ...
    def test_get_many_with_filter(self):
        ...
        selector = ODataTestSelector()
        query = QueryBuilder().filter("is_active eq true")
        dtos = selector.get_many(query)

        assert len(dtos) == 2
        assert all(dto.is_active for dto in dtos)
    ...
    def test_get_many_with_select(self):
        ...
        ODataTestModel.objects.create(name="Test", description="Desc", count=10, created_at=timezone.now())
...
class TestSelectorExistsBy:
    ...
    def test_exists_by_false(self):
        ...
        selector = ODataTestSelector()
        query = QueryBuilder().filter("name eq 'nonexistent'")
    ...
    def test_exists_by_with_filter(self):
        ...
        query_active = QueryBuilder().filter("is_active eq true")
        assert selector.exists_by(query_active)

        query_inactive = QueryBuilder().filter("is_active eq false")
        assert not selector.exists_by(query_inactive)
...
class TestSelectorODataQueryIntegration:
    ...
    def test_from_request_query_string_pattern(self):
        ...
        obj = ODataTestModel.objects.create(name="Target Object", count=100, created_at=timezone.now())
